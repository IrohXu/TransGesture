    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            for b_i in range(len(val_gesture_heatmap_pred)):
                ...
                print(calculate_multiclass_miou(scaled_heatmap, gt_mask, ignore_index=0)[0])
                if calculate_multiclass_miou(scaled_heatmap, gt_mask, ignore_index=0)[0] > 0.3:
                    cv2.imwrite(visualization_save_path, visualization_pred)
                    cv2.imwrite(visualization_save_path.replace(".jpg", "_orginal.jpg"), image)
