...
from .layers import LayerScale, Mlp, PatchEmbed, RMSNorm, RopePositionEmbedding, SelfAttentionBlock, SwiGLUFFN, named_apply
...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        ...
        all_x = x
        output = []
        for idx, (x, masks, rope_i) in enumerate(zip(all_x, masks_list, rope)):
    ...
    def forward_features(self, x: Tensor | List[Tensor], masks: Optional[Tensor] = None) -> List[Dict[str, Tensor]]:
        outputs = {}
        image_features = self.forward_features_list([x], [masks])[0]
        B = x.shape[0]
        H, W = image_features["rope"]
        outputs["last_feat"] = (
            image_features["x_norm_patchtokens"].reshape(B, H, W, -1)
            .permute(0, 3, 1, 2)
        )
